...
bool Position::CheckBoard(){
    ...
    int sq64,t_pce_num,sq120;
    int t_piece;
    int colour;

    //check consistency of typeBB , pieceBB and byColourBB
    
    for (t_piece = wP; t_piece <= bK; ++t_piece){
        for (t_pce_num = 0; t_pce_num < pceNum[t_piece]; ++t_pce_num){
            sq120 = Square(pieceList[t_piece][t_pce_num]);
            assert(piece_on(static_cast<Square>(sq120))==t_piece);
        }
    }

    //check piece count and other counters
    for (sq64 = 0; sq64 < 64; ++sq64){
        t_piece = piece_on(static_cast<Square>(sq64));
        t_pceNum[t_piece]++;
        colour = PieceCol[static_cast<int>(t_piece)];
...
void Position::UpdateListsMaterial(){
    ...
    for (index=0; index< BRD_SQ_NO; ++index){
        ...
        piece = piece_on(sq);
        if (piece!=EMPTY){
            colour = static_cast<Colour>(PieceCol[piece]);
...
bool Position::ParseFen(string fen){
    ...
    while ((rank >= RANK_1) && it < fen.end()){
       ...
       for (i=0; i< count; i++){
           sq64 = Square(rank * 8 + file);
        
           if (piece != EMPTY){
               set_bitboards(sq64, piece);
           }
           file++;
       }
       it++;
...
void Position::PrintBoard(){
    ...
    for (rank = RANK_8; rank >= RANK_1; rank--){
        ...
        for (file = FILE_A; file <= FILE_H; file++){
            sq = FR2SQ(file,rank);
            piece = piece_on(static_cast<Square>(sq));
...
void Position::MirrorBoard(){
    ...
    Colour tempSide = static_cast<Colour>(side^1);
    Piece SwapPieces[13] = {EMPTY,bP,bN,bB,bR,bQ,bK,wP,wN,wB,wR,wQ,wK};
    CastleType tempCastlePerm = NONE;
    ...
    for (sq = 0; sq < 64; sq++){
        ...
        set_bitboards(static_cast<Square>(sq), static_cast<Piece>(tp));
    }
    side = tempSide;
...
bool Position::MakeMove(const Move move){
    ...
    castlePerm &= CastleType(CastlePerm[from]);
    castlePerm &= CastleType(CastlePerm[to]);
    enPas = NO_SQ;
     
    HASH_CA();
    assert(CheckBoard());
    Piece pceCap = captured(move);
    fiftyMove++;

    if(pceCap != EMPTY){
        assert(PieceValid(pceCap));
        ClearPiece(to);
        fiftyMove=0;
        assert(CheckBoard());
    }
    ...
    HASH_SIDE();
    assert(CheckBoard());
    if (SqAttacked(KingSq[t_side],side)){
        assert(side != t_side);
        TakeMove();
        return false;
    }
...
void Position::TakeMove(){
    ...
    if (piece_on(from) == wK || piece_on(from) == bK){
        KingSq[side]=from;
    }

    Piece capPce = captured(move);
    if (capPce != EMPTY){
    ...
    if (prPce != EMPTY){
        assert(PieceValid(prPce) && !PiecePawn[prPce]);
        ClearPiece(from);
        AddPiece(from,PieceCol[prPce] == WHITE ? wP: bP);
    }
...
BitBoard Position::attackers_to(Square sq, BitBoard occupied) const {
    return ((attacks_from<PAWN>(sq, BLACK) & pieces(BLACK, PAWN)) | 
            (attacks_from<PAWN>(sq, WHITE) & pieces(WHITE, PAWN)) |
            (attacks_from<KNIGHT>(sq) & pieces(KNIGHT)) | 
            (attacks_bb<ROOK>(sq, occupied) & BitBoard(pieces(ROOK) | pieces(QUEEN))) | 
            (attacks_bb<BISHOP>(sq, occupied) & BitBoard(pieces(ROOK) | pieces(QUEEN))) | 
