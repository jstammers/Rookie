...
BitBoard PawnAttacksBB[2][BRD_SQ_NO];
BitBoard PseudoAttacksBB[PIECE_NO][BRD_SQ_NO];
...
namespace bitboards{
    void init()
    {
        Rank1BB.PrintBitBoard();
        Rank2BB.PrintBitBoard();
        Rank3BB.PrintBitBoard();
        Rank4BB.PrintBitBoard();
        Rank5BB.PrintBitBoard();
        Rank6BB.PrintBitBoard();
        Rank7BB.PrintBitBoard();
        Rank8BB.PrintBitBoard();

        FileABB.PrintBitBoard();
        FileBBB.PrintBitBoard();
        FileCBB.PrintBitBoard();
        FileDBB.PrintBitBoard();
        FileEBB.PrintBitBoard();
        FileFBB.PrintBitBoard();
        FileGBB.PrintBitBoard();
        FileHBB.PrintBitBoard();
        Square sq, tsq;
        File f;
        Rank r;
        for (sq = A1; sq <= H8; ++sq)
        ...
        for (Colour c = WHITE; c <= BLACK; ++c)
        {
            for (sq = A1; sq <= H8; ++sq)
            {
                PawnAttacksBB[c][sq] = BitBoard(0ULL);
            }
        }
        for (auto pt : Pieces)
        {
            for (sq = A1; sq <= H8; ++sq)
            {
                PseudoAttacksBB[pt][sq] = BitBoard(0ULL);
            }
        }
            int steps[][4] = {{}, {7, 9}, {6, 10, 15, 17}, {}, {}, {}, {1, 7, 8, 9}};
            ...
            for (PieceType pt : {PAWN, KNIGHT, KING})
                {
                    ...
                    {
                        Square to = sq + Square(c == WHITE ? steps[pt][i] : -steps[pt][i]);
                        if (SqOnBoard(to) && distance(sq,to) < 3)
                        {
                            if (pt == PAWN)
                                PawnAttacksBB[c][sq] |= to;
                            else
                                PseudoAttacksBB[pt][sq] |= to;
                        }
                    }
                }
            }
...
namespace {
    ...
    {
        //TODO Fix this during magic bitboard generation
        BitBoard attack = 0;
        for (int i = 0; i < 4; i++)
        {
    ...
    // This uses the "fancy approach" to enerate them.
    void init_magics(BitBoard table[], MagicBB magics[], Square deltas[])
    {
         ...
         for (Square sq= A1; sq <= H8; ++sq)
         {
            ...
            std::cout << "Rank8" << endl;
            Rank8BB.PrintBitBoard();
            std::cout <<"NotRank" << endl;
            BitBoard(~rank_bb(sq)).PrintBitBoard();
            std::cout << "Rank" << endl;
            rank_bb(sq).PrintBitBoard();

            std::cout << "FileA" << endl;
            FileABB.PrintBitBoard();
            std::cout << "File H" << endl;
            FileHBB.PrintBitBoard();
            std::cout << "Not File" << endl;
            BitBoard(~file_bb(sq)).PrintBitBoard();
            std::cout << "File" << endl;
            file_bb(sq).PrintBitBoard();
            //For a square sq, the mask of all sliding attacks from sq are calculated on an empty board
            MagicBB &m = magics[sq];
            m.mask = sliding_attack(deltas, sq, 0) & BitBoard(~edges);
            std::cout << sq << endl;
            m.mask.PrintBitBoard();
